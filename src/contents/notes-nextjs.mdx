...
createdTime: '2025-09-28 11:27:23'
...
并且当服务端组件中包含客户端组件，会触发 next 的**边界处理**。

服务端组件会将 props **序列化**，然后传递给 客户端组件。这也就意味着**无法序列化**的数据是无法传递的。

因此客户端组件通常是不允许传递事件回调，类似：onClick、onSubmit。
